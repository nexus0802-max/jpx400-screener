...
def fetch_benchmark_weighted_return():
    """TOPIX連動ETFの加重リターンを取得。取得できない場合はNoneを返し、
    その場合はRSは「銘柄間の相対順位のみ」（超過リターンではなく素の加重リターンの順位）にフォールバックする。"""
    try:
        ...
        return calc_weighted_return(closes)
    except Exception as e:
...
def main():
    ...
    bench_return = fetch_benchmark_weighted_return()
    ...
    with open("docs/data_v2_phase2.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False)

    print(f"\n完了: {len(all_results)}銘柄 -> docs/data_v2_phase2.json")
